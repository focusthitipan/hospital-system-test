...
async function claimInsurance(patientId: number, treatmentCost: number): Promise<boolean> {
  ...
  try {
    ...
    //
    const result = await client.query<{ insurance_limit: string }>(
...
async function getLimit(patientId: number): Promise<number> {
  const r = await pool.query(
    'SELECT insurance_limit FROM patients WHERE id = $1', [patientId]
  );
...
async function runTests(): Promise<void> {
  ...
  try {
    ...
    const t5 = await claimInsurance('1 OR 1=1' as any, 100);
