...
function canOverride(doctorRole, required) {
...
async function recordAllergy(patientId, allergy, recordedBy) {
    const r = await pool.query(`INSERT INTO drug_allergies (patient_id, drug_id, drug_class, severity, reaction, recorded_by)
     VALUES ($1, $2, $3, $4, $5, $6) RETURNING allergy_id`, [patientId, allergy.drugId ?? null, allergy.drugClass ?? null,
...
async function checkAllergyAlerts(patientId, drugId) {
 ...
 */
async function prescribeDrug(patientId, drugId, dosage, durationDays, doctorId) {
    try {
        const r = await pool.query(`INSERT INTO prescriptions (patient_id, drug_id, dosage, duration_days, prescribed_by)
       VALUES ($1, $2, $3, $4, $5) RETURNING prescription_id`, [patientId, drugId, dosage, durationDays, doctorId]);
        ...
        if (err.message?.startsWith('ALLERGY_BLOCK')) {
            ...
            throw new AllergyError({
                ...
                requiredOverrideRole: REQUIRED_OVERRIDE_ROLE[severity],
            });
        }
        throw err;
    }
 ...
 */
async function overrideAndPrescribe(patientId, drugId, allergyId, dosage, durationDays, doctorId, reason) {
    // ── 1. โหลดข้อมูลแพทย์และ allergy ──────────────────────────────────────
    const [docRes, allergyRes] = await Promise.all([
        pool.query('SELECT role FROM doctors WHERE doctor_id = $1', [doctorId]),
        pool.query('SELECT severity, patient_id FROM drug_allergies WHERE allergy_id = $1', [allergyId]),
    ]);
    if (docRes.rows.length === 0)
        throw new Error(`Doctor ${doctorId} not found`);
    if (allergyRes.rows.length === 0)
        throw new Error(`Allergy ${allergyId} not found`);
    ...
    if (!canOverride(doctorRole, requiredRole)) {
        throw new PermissionError(`แพทย์ role "${doctorRole}" ไม่มีสิทธิ์ Override allergy severity "${severity}" ` +
    ...
    try {
        ...
        const overrideRes = await client.query(`INSERT INTO override_logs (patient_id, drug_id, allergy_id, overridden_by, override_reason)
       VALUES ($1, $2, $3, $4, $5) RETURNING override_id`, [patientId, drugId, allergyId, doctorId, reason]);
        const overrideId = overrideRes.rows[0].override_id;
        // ── 4. INSERT prescription พร้อม override_id → Trigger จะข้ามการตรวจ
        const prescRes = await client.query(`INSERT INTO prescriptions
         (patient_id, drug_id, dosage, duration_days, prescribed_by, override_id)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING prescription_id`, [patientId, drugId, dosage, durationDays, doctorId, overrideId]);
...
async function runTests() {
    ...
    try {
        ...
        assert(e.alert.requiredOverrideRole === 'head', `anaphylaxis requires head → ${e.alert.requiredOverrideRole}`);
    }
    ...
    try {
        await overrideAndPrescribe(1, 1, allergyId, '500mg x3/day', 7, 1, 'Patient needs this drug');
        assert(false, 'standard should not override severe');
    }
    catch (e) {
        assert(e instanceof PermissionError, `PermissionError: standard cannot override severe → ${e.message}`);
    }
    // Senior doctor มีสิทธิ์
    const { prescriptionId, overrideId } = await overrideAndPrescribe(1, 1, allergyId, '500mg x3/day', 7, 2, 'Benefits outweigh risks — patient requires Amoxicillin for resistant infection');
    ...
    console.log('═'.repeat(62));
    try {
        await pool.query(`INSERT INTO prescriptions (patient_id, drug_id, dosage, duration_days, prescribed_by)
       VALUES (2, 1, '500mg', 7, 1)` // Bob + Amoxicillin (penicillin class allergy)
        );
        assert(false, 'trigger should have blocked this INSERT');
    }
    catch (e) {
        assert(e.message.includes('ALLERGY_BLOCK'), `DB trigger blocked direct INSERT → "${e.message.split('|')[0]}"`);
    }
