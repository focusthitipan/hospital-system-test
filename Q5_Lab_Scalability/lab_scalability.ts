...
type Resolution  = 'thumbnail' | 'mobile' | 'tablet' | 'original';
...
// คุณสมบัติของแต่ละ resolution variant
interface ResolutionSpec {
  width: number; height: number;
  jpegQuality: number;       // % (0–100)
  estimatedKb: number;       // ขนาดโดยประมาณสำหรับภาพ X-Ray ขนาดมาตรฐาน
  cacheMaxAgeSec: number;    // CDN cache TTL
  cacheDirective: string;    // HTTP Cache-Control header
}
...
const RESOLUTION_SPECS: Record<Resolution, ResolutionSpec> = {
...
function selectResolution(deviceType: 'mobile' | 'tablet' | 'workstation'): Resolution {
  if (deviceType === 'workstation') return 'original';
...
async function checkDoctorAccess(doctorId: number, labResultId: number): Promise<boolean> {
...
function signToken(
  tokenId: string, imageId: number, doctorId: number,
  deviceIp: string, expiresAt: string,
): string {
...
async function requestSignedUrl(params: {
...
}): Promise<{ tokenId: string; url: string; expiresAt: Date; cacheDirective: string } | null> {
  ...
  const imgRes = await pool.query<{ resolution: string }>(
    'SELECT resolution FROM lab_images WHERE image_id = $1', [imageId]
  );
  if (imgRes.rows.length === 0) return null;
  const resolution = imgRes.rows[0].resolution as Resolution;
...
async function validateToken(
  tokenId: string, requestingIp: string,
): Promise<{ result: AccessResult; storagePath?: string; cacheDirective?: string }> {

  const r = await pool.query<{
    image_id: number; doctor_id: number; resolution: string;
    device_ip: string; hmac_sig: string; expires_at: Date;
    is_used: boolean; storage_path: string;
  }>(`
  ...
  const expiresStr = t.expires_at instanceof Date
    ...
    : new Date(t.expires_at as unknown as string).toISOString();
  ...
  return {
    ...
    cacheDirective: RESOLUTION_SPECS[t.resolution as Resolution].cacheDirective,
  };
}
...
async function getAuditReport(imageId: number): Promise<void> {
  const r = await pool.query(`
    SELECT
      l.log_id,
      d.name       AS doctor,
...
async function runTests(): Promise<void> {
  ...
  assert(!token2!.url.includes('alice') && !token2!.url.includes('patient'),
    'URL leaks no patient identity');
  ...
  const expiredTokenId = randomBytes(16).toString('hex');
  const pastTime       = new Date(Date.now() - 60_000); // 1 นาทีที่แล้ว
  ...
  try {
    ...
    assert(e.message.includes('AUDIT_LOG_IMMUTABLE'),
      `trigger blocks UPDATE: "${e.message}"`);
  }
