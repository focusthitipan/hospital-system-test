...
// ─── Functions ────────────────────────────────────────────────────────────────
function buildUserPrompt(patientText) {
...
async function extractSymptoms(patientText) {
...
function validateExtraction(data, path = 'root') {
    const violations = [];
    function scan(obj, p) {
...
async function runTests() {
    ...
    assert(result.recent_food.some(f => f.includes('ส้มตำ')), `food contains "ส้มตำ": ${JSON.stringify(result.recent_food)}`);
    // ── Test 4: Severity = null (not stated by patient) ──────────────────────
    console.log('\n─'.repeat(64).slice(1));
    console.log('Test 4: Severity = null (patient did not state severity)');
    console.log('─'.repeat(64));
    assert(sym?.severity === null, `severity is null (patient did not say "มาก/น้อย"): ${sym?.severity}`);
    assert(result.current_medications === null, `medications = null (not mentioned): ${result.current_medications}`);
    assert(result.allergies_mentioned === null, `allergies = null (not mentioned): ${result.allergies_mentioned}`);
    ...
    if (validation.valid) {
        ...
        assert(!JSON.stringify(result).toLowerCase().includes('food poison'), 'No "food poison" language in output');
        assert(!JSON.stringify(result).toLowerCase().includes('อาหารเป็นพิษ'), 'No "อาหารเป็นพิษ" in output');
        assert(!JSON.stringify(result).toLowerCase().includes('gastro'), 'No "gastroenteritis" in output');
    }
    else {
    ...
    console.log('─'.repeat(64));
    assert(result.source_text === INPUT, `source_text matches input: "${result.source_text}"`);
