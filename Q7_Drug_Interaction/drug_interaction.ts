...
type Severity       = 'contraindicated' | 'major' | 'moderate' | 'minor' | 'none';
type DecisionOutcome = 'blocked' | 'auto_advisory' | 'requires_review' | 'escalated' | 'safe';

// ผลวิเคราะห์จาก AI — ต้องมี confidence_score เสมอ
interface AIAnalysis {
  ...
  confidence_score:     number;         // 0.0 – 1.0  (หัวใจของ Human-in-the-Loop)
...
interface InteractionResult {
  drug_a:           string;
  drug_b:           string;
  db_severity:      Severity | null;     // null = ไม่มีใน DB
...
async function checkDatabase(drugA: string, drugB: string): Promise<{
  severity: Severity; mechanism: string; clinical_effect: string; recommendation: string;
} | null> {
...
async function analyzeWithAI(drugA: string, drugB: string): Promise<AIAnalysis> {
...
// ─── Step 3: Safety Decision Engine ──────────────────────────────────────────
//
//  กฎสำคัญ: DB เป็น authoritative เสมอ AI ไม่สามารถ override DB contraindication ได้

type DbRow = { severity: Severity; mechanism: string; clinical_effect: string; recommendation: string; } | null;

function makeDecision(
  dbResult: DbRow,
  ai:       AIAnalysis,
): { decision: DecisionOutcome; message: string; routed: boolean } {

  // ── Priority 1: DB ตัดสินเสมอสำหรับ contraindicated / major ──────────────
  if (dbResult?.severity === 'contraindicated') return {
    decision: 'blocked',
    message:  `[DB] CONTRAINDICATED: ${dbResult.clinical_effect}. ${dbResult.recommendation}`,
    routed:   false,
  };
  if (dbResult?.severity === 'major') return {
    decision: 'blocked',
  ...
  if (!ai.interaction_found) return {
    decision: 'safe',
...
function notifyForReview(checkId: number, drugA: string, drugB: string,
                         outcome: DecisionOutcome, confidence: number): void {
...
async function logCheck(entry: {
  drugA: string; drugB: string; dbSeverity: string | null;
  aiConfidence: number | null; decision: string; safetyMessage: string;
}): Promise<number> {
  const r = await pool.query<{ check_id: number }>(`
...
async function checkInteraction(drugA: string, drugB: string): Promise<InteractionResult> {
  ...
  const safeAI = aiResult ?? {
    interaction_found: false, severity: 'none' as Severity,
...
async function runTests(): Promise<void> {
  ...
  assert(t3.decision !== 'safe' || (t3.ai_analysis?.confidence_score ?? 0) >= 0.80,
    `decision appropriate for AI confidence ${((t3.ai_analysis?.confidence_score ?? 0)*100).toFixed(0)}%`);
  ...
  assert(['auto_advisory','requires_review','escalated'].includes(t4.decision),
    `decision is advisory/review/escalated: ${t4.decision}`);
